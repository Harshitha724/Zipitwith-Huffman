...
void huffman::createArr() {
    for(int i=0;i<128;i++){ //128 nodes for each ASCII character
...
//reads input file and builds freq map using arr[]
void huffman::createMinHeap(){
    ...
    for(int i=0;i<128;i++){ //only freq>0 are pushed
        if(arr[i]->freq>0){
            minHeap.push(arr[i]);}}}

//Builds huffman tree using temporary priority queue
...
void huffman::saveEncodedFile(){
    ...
    //save Huffman table (char and 16type path for each)
    priority_queue <Node*, vector<Node*>, Compare> tempPQ(minHeap);
    while(!tempPQ.empty()){
    ...
    //save encoded data and final padding info
	in += (char)binToDec(s);
    in += (char)count;
...
//Reconstructs the huffman tree from the stored header in the encoded file
